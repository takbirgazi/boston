            ...
            <div className="flex flex-col gap-3 border-b border-gray-300 pb-5 m-4">
                ...
                <div className="flex items-center gap-5">
                    <p className="flex items-center gap-2">
                        <FaUser className="text-xs text-[#1ecb15]" />
                        <span className="font-medium text-gray-800">{rentCardData.specifications.seat}</span>
                    </p>
                    <p className="flex items-center gap-2">
                        <TfiBag className="text-xs text-[#1ecb15]" />
                        <span className="font-medium text-gray-800">{rentCardData.specifications.luggage}</span>
                    </p>
                    <p className="flex items-center gap-2">
                        <GiCarDoor className="text-xs text-[#1ecb15]" />
                        <span className="font-medium text-gray-800">{rentCardData.specifications.door}</span>
                    </p>
                    <p className="flex items-center gap-2">
                        <FaCar className="text-xs text-[#1ecb15]" />
                        <span className="font-medium text-gray-800">{rentCardData.specifications.body}</span>
                    </p>
                </div>
            </div>
            <div className="flex justify-between items-center m-4">
                <div>
                    <p className="text-gray-600 text-sm font-medium pb-0.5">Daily Rate From</p>
                    <h2 className="text-3xl font-semibold font-poppins text-gray-800">$ {rentCardData.dailyRate}</h2>
                </div>
                <div>
                    <button className="px-4 py-1.5 rounded-sm bg-[#1ecb15] text-white font-semibold cursor-pointer">Rent Now</button>
                </div>
            </div>
        </div>
    );
